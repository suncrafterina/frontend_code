...
export class ShippingListComponent implements OnInit {
	...
	startPage = 0;
	endPage = 0;
	displayedColumns: string[] = ['owner_name', 'email', 'phone_number', 'commission','reg_date', 'status',  'action'];
  ...
  constructor(public dialog: MatDialog,public translate: TranslateService, private _messageService: MessageService ,private vendorsService: VendorsService, private _comm: CommunicationService, private _alrt: AlertService, private router: Router, private _auth: AuthenticationService) {
	...
	**/
	fetchCategoryList() {
		...
		this.vendorsService.getShipperList(this.filter, postData).subscribe((data) => {
		this._comm.notifyShowHideLoader({ show: false });
    	this.shipperList = data.body;
	...
	updateCommission(result) {
			if(result.id){
			  // this._comm.notifyShowHideLoader({ show: true });
			  this.vendorsService.updateShipperCommission(result).subscribe(res => {
				  // this._comm.notifyShowHideLoader({ show: false });
				  this.fetchCategoryList();
	...
	openDialog(element: any) {
		const dialogRef = this.dialog.open(CommissionMatDialogComponent, {
		  width: '400px',
