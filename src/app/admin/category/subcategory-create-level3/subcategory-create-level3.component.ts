...
@Component({
  selector: 'app-subcategory-create-level3',
  templateUrl: './subcategory-create-level3.component.html',
  styleUrls: ['./subcategory-create-level3.component.css']
})
export class SubcategoryCreateLevel3Component implements OnInit {
  ...
  formInIt() {
    this.categoryForm = this.fb.group({
      ...
      sorting_order: ['', [Validators.required, CustomValidation.validateWhiteSpace, CustomValidation.validateNumberRequired,CustomValidation.validateNumberIntegerRequired]],
      category_level: ['', [Validators.required]],
      parent_id: ['', [Validators.required]],
      image_file:[''],
      icon_file:['']
    });
    this.categoryForm.controls.category_level.setValue('LEVEL_THREE');
    this.categoryForm.controls.parent_id.setValue(this.subcategoryId );
  ...
  addCategory() {
    if (this.categoryForm.valid) {
      ...
      this._category.addCategory(postData,this.selectFile,this.companySelectFile).subscribe(res => {
        ...
        this.router.navigate(["/admin/category/subcategory-list-level3/"+this.categoryId+'/'+this.subcategoryId]);
      }, error => {
        this._comm.notifyShowHideLoader({ show: false });
  ...
  getCategoryDetails() {
    if (this.subCatId) {
      this._comm.notifyShowHideLoader({ show: true });
      this._category.getCategoryDetails(this.subCatId).subscribe(res => {
        this._comm.notifyShowHideLoader({ show: false });
        if (res) {
          var resp = res;
          if (res['image_file']) {
  ...
  updateCategory() {
    if (this.categoryForm.valid && this.subCatId) {
			const postData = JSON.parse(JSON.stringify(this.categoryForm.value));
			postData['id'] = this.subCatId;
			this._comm.notifyShowHideLoader({ show: true });
