...
export class SubcategoryCreateComponent implements OnInit {
  ...
  formInIt() {
    ...
    this.categoryForm.controls.category_level.setValue('LEVEL_TWO');
    this.categoryForm.controls.parent_id.setValue(this.categoryId);
  }
  hasError(field: any) {
  ...
  addCategory() {
    if (this.categoryForm.valid) {
      ...
      this._category.addCategory(postData,this.selectFile,this.companySelectFile).subscribe(res => {
        ...
        this.router.navigate(["/admin/category/subcategory-list/"+this.categoryId]);
      }, error => {
        this._comm.notifyShowHideLoader({ show: false });
  ...
  updateCategory() {
    if (this.categoryForm.valid && this.subcategoryId) {
			const postData = JSON.parse(JSON.stringify(this.categoryForm.value));
			postData['id'] = this.subcategoryId;
			this._comm.notifyShowHideLoader({ show: true });
