...
export class LocationComponent implements OnInit {
  ...
  // country:any = [];
  // zip_code:any = [];
	constructor(private _shipping: shippingService,private _messageService: MessageService, public translate: TranslateService, private _comm: CommunicationService, private _alrt: AlertService, private router: Router, private route: ActivatedRoute, private _auth: AuthenticationService) {
	...
	ngOnInit(): void {
		this._messageService.changeMessage('locations');
		setTimeout(() =>
	...
	**/
    ...
    this._comm.notifyShowHideLoader({ show: true });
		this._shipping.getShippingLocationList(this.filter, postData).subscribe((data) => {
	...
	onValChange(event, data){
		...
		let id=data.id;
		this._comm.notifyShowHideLoader({ show: true });
		this._shipping.getShippingStatus(id, postData).subscribe((data) => {
		this.fetchCategoryList()
		this._comm.notifyShowHideLoader({ show: false });
    	}, error => {
			this._comm.notifyShowHideLoader({ show: false });
