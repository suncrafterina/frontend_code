...
export class TransactionsComponent implements OnInit {
  ...
  displayedColumns: string[] = ['id', 'customer_name', 'commission_amount', 'total_amount', 'txn_date', 'action'];
  totalRecords: any;
  fetchedRecords = 1;
  startPage = 0;
  endPage = 0;
  ...
  fetchOrderList() {
    ...
    this._sale.getTransactionList(this.filter, postData).subscribe(data => {
  ...
  {
    this._comm.notifyShowHideLoader({ show: true });
    const postData = {user_type:'ROLE_FACTORY_VENDOR',from_to: this.fromDate, to_date: this.toDate };
    this._sale.getDownloadTransation(postData).subscribe({
      next: (response: any) => {
